...
void destroyNode(BSTNode* node) { //borra el nodo sin importar lo que hay debajo?
    BSTNode* left = newNode(node->left->key, node->left->value);
    BSTNode* right = newNode(node->right->key, node->right->value);
